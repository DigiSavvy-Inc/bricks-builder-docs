...
date: 2026-01-05T11:09:15.691399
status: success
---
...
- [Query Loop](#query-loop)
  ...
  - [Query Type: Array](#query-type-array)
      - [Use Case 1: Looping Nested Arrays from API Responses](#use-case-1-looping-nested-arrays-from-api-responses)
      - [Use Case 2: Looping Custom PHP Arrays](#use-case-2-looping-custom-php-arrays)
    - [Dynamic Data Tag: {query_array}](#dynamic-data-tag-queryarray)
      - [Example 1: Flat List](#example-1-flat-list)
      - [Example 2: Associative Array](#example-2-associative-array)
      - [Example 3: Nested Arrays](#example-3-nested-arrays)
  - [Results Filter](#results-filter)
      - [Example: Filter by Age](#example-filter-by-age)
...
TheArrayquery loop type lets you loop through any PHP or JSON array.

This is especially useful for:

- Rendering API response arrays. (API queryloop)
...
If your API response contains nested arrays (e.g.cars: ['BMW', 'Mazda']), you can easily loop through them by using anested array loopand setting the correct path in the Array Editor.
...
To loop through eachcarsarray inside the main loop:

`cars`

1. Nest anotherArray Loopinside the parent loop.
2. In the nested loop’sArray Editor, set:{query_array @key:'cars'}
3. Use{query_array}inside the nested loop to output each car.

`{query_array @key:'cars'}`

`{query_array}`

### Results Filter

The“Results Filter”feature allows you to filter loop results using conditionsafter the array is fetched but before rendering begins(similar to thebricks/query/resultPHP filter).
...
You only want to display people whoseage is less than 50. You can configure a result filter like this:
...
This filter will exclude the last item (Pony, age 66) from rendering, so onlyJohnandAlanwill appear in the loop output.
...
You can add multiple filter rules if needed—each rule will be evaluated withANDlogic. Usebricks/query/resulthook for more complex filtering.
