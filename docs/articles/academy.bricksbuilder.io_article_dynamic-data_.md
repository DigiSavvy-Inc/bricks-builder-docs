...
date: 2026-01-05T11:08:43.473090
status: success
---
...
- [Dynamic Data](#dynamic-data)
  ...
  - [Date Dynamic Tags](#date-dynamic-tags)
    - [Current Date fields](#current-date-fields)
    - [Format Date](#format-date)
        - [Example 1: Simple Date format conversion](#example-1-simple-date-format-conversion)
        - [Example 2: Format from API Response](#example-2-format-from-api-response)
        - [Example 3: Convert to Timestamp](#example-3-convert-to-timestamp)
...
{query_loop_index}– Returns the index of the current loop item, starting at 0. Use the@start-atargument to start the index from any other number. Example:{query_loop_index @start-at:1}to start the index at 1 instead of 0. The@padargument lets you define a 0-based padding. Using0results in an index output of001and010instead of1and10.

`{query_loop_index}`

`@start-at`

`{query_loop_index @start-at:1}`
...
- @from: The input format (e.g.,Y-m-d,Y/m/d,Y-m-d\TH:i:s.Z\Z, etc.)
...
`{query_api @key:'registered'}`

{format_date @date:'{query_api @key:'registered'}' @from:'Y-m-d\TH:i:s.Z\Z' @to:'Y m d (l)'}
...
- Example:{acf_page_builder_cta_modul_options_css_id @fallback:'{acf_get_row_layout}-{query_loop_index}'}. Ifacf_page_builder_cta_modul_options_css_idis empty, use current ACF flexible layout concatenate with current loop index as return string. Good use case when using this on dynamic CSS ID.

`{acf_text_field @fallback:'This is the fallback text!'}`

`acf_text_field`

`{acf_page_builder_cta_modul_options_css_id @fallback:'{acf_get_row_layout}-{query_loop_index}'}`

`acf_page_builder_cta_modul_options_css_id`
