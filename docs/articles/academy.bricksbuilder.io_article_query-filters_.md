...
date: 2026-01-05T11:09:06.583681
status: success
---
...
- Custom field support: By default, Bricks Query Filters work only with simple, plain-text custom field values. They do not support fields that store data in serialized formats. (Please read thisCustom Fields Integrationif you are using ACF or Metabox.io, we improved this in1.11.1)
- Component limitation: Filter elements must not be used inside a Bricks component. Similarly, the target query loop must not be a loop that lives inside a componentunlessthe component’s root is the query loop itself.Read more
