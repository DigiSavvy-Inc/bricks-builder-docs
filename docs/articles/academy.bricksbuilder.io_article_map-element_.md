...
date: 2026-01-05T11:08:57.433384
status: success
---
...
Use this mode to feed the map with addresses/coordinates from a query.

For example, if you have acustom post typelike “Properties” where each property contains an address, enabling the Query loop allows you to show markers of all property addresses on the map.

You can also define acustom Info Boxusing a popup template.

In our example above, we selected the custom post typeproperty, where each address is stored in a custom field (i.e. ACF text fields). Providing the latitude & longitude coordinates will be more accurate than a physical address.
...
Note:It’s strongly recommended to providelatitude and longitudevalues whenever available. If you rely solely on addresses and require geocoding every time markers are rendered, you may incur charges from the Google Maps Platform Geocoding API.
